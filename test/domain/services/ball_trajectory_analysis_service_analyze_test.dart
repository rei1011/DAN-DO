...
class FakeBallDetector extends BallDetector {
  FakeBallDetector(this._script)
    : super(yolo: YOLO(modelPath: 'fake-model-for-test', task: YOLOTask.detect));
...
void main() {
  ...
  group('BallTrajectoryAnalysisService.analyze (ROIループの結線)', () {
    test(
      'フルフレームではなくタップ位置中心のクロップが使われ、'
      'オフセット変換を経て妥当なShotResultが得られる(初期クロップ→追跡クロップの遷移)',
      () async {
    ...
    test(
      '5フレーム連続でロストするとフルフレーム探索にフォールバックし、'
      '再検出後はクロップ探索に復帰する',
      () async {
        // idx0: アドレス確立(クロップ240)
    ...
    test(
      '信頼度がconfidenceThreshold未満の検出のみでroiCursorがnullのフレームは'
      'ArgumentErrorを投げずスキップされ、後続フレームで正常に追跡が続く',
      () async {
